...
{
    public partial class DoctorUpdateForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        public DoctorUpdateForm(string _tcno, int _id, string _name, string _surname, string _gender, string _date, string _mail, string _phonenumber, string _policlinicName, string _username)
        {
            InitializeComponent();
        ...
        private async void DoctorUpdateForm_Load(object sender, EventArgs e)
        {
            tcProperty();
            otherProperty();
            ScreenProperty();
        ...
        {
            txtDoctorTcNo.Text = tcno.ToString();
            txtDoctorName.Text = name.ToString();
            txtDoctorSurname.Text = surname.ToString();
            txtDoctorMail.Text = mail.ToString();
            txtDoctorPhoneNumber.Text = phonenumber.ToString();
            txtDoctorUsernameUpdate.Text = username.ToString();
            DropdownDoctorGender.SelectedItem = gender.ToString();
            DropDownPoliclinic.Text = policlinicName.ToString();
            DatePickerDoctorBorn.Text = date;
        }
        ...
        private void LabelDoctorDarkOpen()
        {
            ...
            labelDoctorUsernameUpdate.ForeColor = Color.White;
        ...
        private void LabelDoctorDarkClose()
        {
            ...
            labelDoctorUsernameUpdate.ForeColor = Color.Black;
        ...
        {
            txtDoctorUsernameUpdate.ForeColor = Color.FromArgb(249, 249, 249);
            txtDoctorUsernameUpdate.FillColor = Color.FromArgb(38, 38, 38);
            txtDoctorUsernameUpdate.BorderColorActive = Color.FromArgb(124, 86, 216);
            txtDoctorUsernameUpdate.BorderColorHover = Color.FromArgb(167, 114, 242);
            txtDoctorUsernameUpdate.BorderColorIdle = Color.Gray;
            txtDoctorUsernameUpdate.OnIdleState.FillColor = Color.FromArgb(38, 38, 38);
            txtDoctorUsernameUpdate.OnHoverState.FillColor = Color.FromArgb(38, 38, 38);
            txtDoctorUsernameUpdate.OnActiveState.FillColor = Color.FromArgb(38, 38, 38);
        }
        private void TxtDoctorUsernameDarkClose()
        {
            txtDoctorUsernameUpdate.ForeColor = Color.FromArgb(26, 26, 26);
            txtDoctorUsernameUpdate.FillColor = Color.White;
            txtDoctorUsernameUpdate.BorderColorActive = Color.FromArgb(230, 230, 230);
            txtDoctorUsernameUpdate.BorderColorHover = Color.FromArgb(235, 235, 235);
            txtDoctorUsernameUpdate.BorderColorIdle = Color.Gray;
            txtDoctorUsernameUpdate.OnHoverState.FillColor = Color.White;
            txtDoctorUsernameUpdate.OnIdleState.FillColor = Color.White;
            txtDoctorUsernameUpdate.OnActiveState.FillColor = Color.White;
        }
        ...
        private void doctorUsername_TextChanged(object sender, EventArgs e)
        {
            ...
            string currentText = txtDoctorUsernameUpdate.Text;
            ...
            {
                int cursorPosition = txtDoctorUsernameUpdate.SelectionStart;
                txtDoctorUsernameUpdate.Text = filteredText;
                txtDoctorUsernameUpdate.SelectionStart = Math.Min(cursorPosition, filteredText.Length); // kücük olanı seçer
            }
        }
        ...
        private async void btn_DoctorUpdate_Click(object sender, EventArgs e)
        {
            ...
            string username = txtDoctorUsernameUpdate.Text.Trim();
                ...
                !(username == null) && pictureBoxDoctorMailRulesCorrect.Visible)
            {
                ...
                else if (policlinicId == 0)
                {
                    MessageBox.Show("Seçili Bir Poliklinik Yok");
                }

                await DoctorKayit(tc, ad, soyad, mail, telefon, cinsiyet, dogumTarihi, id, policlinicId, username);
            }
            else
            {
        ...
        private async Task PoliclinicCategoryLoad()
        {
            ...
            DropdownDoctorGender.SelectedItem = id;
            foreach (var item in DropDownPoliclinic.Items)
            {
                PoliclinicCategory category = item as PoliclinicCategory;
                if (category != null && category.Name == policlinicName)
                {
                    policlinicId = category.Id;
                    break;
                }
            }
        }
        ...
        public async Task DoctorKayit(string tc, string ad, string soyad, string mail, string telefon, string cinsiyet, DateTime dogumTarihi, int doctorID, int policlinicId, string username)
        {
            string operation = "update";
            string query = @"UPDATE Doctors SET TcNo = @TcNo, Name = @Name, Surname = @Surname, Gender = @Gender, [Date] = @Date, Email = @Email, PhoneNumber = @PhoneNumber, Username =@Username, PoliclinicId=@PoliclinicId WHERE Id = @Id;";
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        {
                            MessageBox.Show("Doktor güncellemesi yapılamadı!");
                        }
                    }
                }
            }
        ...
        private void DoctorAddForm_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                btn_DoctorUpdate_Click(sender, e);
        ...
        private void labelRandomUsername_Click(object sender, EventArgs e)
        {
            if (txtDoctorName.Text != "" && txtDoctorName.Text != null && txtDoctorSurname.Text != "" && txtDoctorSurname.Text != null)
            {
                ...
                txtDoctorUsernameUpdate.Text = userName;
            }
            else
            {
